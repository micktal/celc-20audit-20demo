...
export default function AgencyDetailsPanel({
  ...
  return (
    ...
    >
      ...
      <div className="mb-3">
        <div className="inline-block px-2 py-1 text-xs rounded-full bg-[hsl(var(--light-grey))]">
          Statut: <span className="font-semibold ml-2">{agency.status}</span>
        </div>
      </div>

      <div className="text-sm text-[hsl(var(--fiducial-grey))] space-y-2">
        <div>
          <strong>Coordonnées:</strong> {agency.lat.toFixed(4)},{" "}
          {agency.lng.toFixed(4)}
        </div>
        <div>
          <strong>Dernier audit:</strong> {lastAudit}
        </div>
        <div>
          <strong>Nombre d'écarts:</strong> {findings}
        </div>
      </div>
